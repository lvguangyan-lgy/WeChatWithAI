...
public class MessageTools {
	...
	public static boolean sendPicMsgByUserId(String userId, String filePath) {
		...
		boolean gif = filePath.toLowerCase().endsWith(".gif");
		if (responseObj != null) {
			String mediaId = responseObj.getString("MediaId");
			if (mediaId != null) {
				return gif ? sendGif(userId, mediaId) : webWxSendMsgImg(userId, mediaId);
			}
		}
	 ...
	 * @return
	 */
	public static boolean sendGif(String userId,String mediaId) {
		String url = String.format("%s/webwxsendemoticon?fun=sys&f=json&pass_ticket=%s", core.getLoginInfo().get("url"),
	...
	private static JSONObject webWxUploadMedia(String filePath) {
		...
		params.put("uploadmediarequest", JSON.toJSONString(paramMap));
		params.put("webwx_data_ticket", webwxDataTicket);
		params.put("pass_ticket", passTicket);
		int chunks = new BigDecimal(fileSize).divide(new BigDecimal(CHUNK_SIZE), 0, BigDecimal.ROUND_UP).intValue();
		long total = 0;
		for(int chunk = 0; chunk < chunks; chunk++) {
			...
			String result = myHttpClient.httpPostFile(url, params, f, start, size);
